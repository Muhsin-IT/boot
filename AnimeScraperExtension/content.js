...
const waitForIframe = () => new Promise((resolve, reject) => {
    ...
    const interval = setInterval(() => {
        if (document.querySelector(IFRAME_SELECTOR)) {
            ...
            resolve(true); 
        } else if (Date.now() - startTime > 15000) { // WAKTU PEMULIHAN 15 DETIK
            clearInterval(interval);
            reject(new Error("RECOVERY FAILED: Iframe gagal muncul kembali setelah 15 detik."));
        }
    }, 500);
});
...
function scrapeDetailData() {
    const data = { 
        judul: 'N/A', link_poster: 'N/A', rating: 'N/A', 
        deskripsi: 'N/A', genre: [], detail_anime: {}, list_episode: [] 
    };
    
    try {
        const infoSection = document.querySelector(INFO_AREA_SELECTOR);
        if (!infoSection) throw new Error("Blok informasi anime utama tidak ditemukan.");

        // Judul & Poster
        data.judul = infoSection.querySelector('h2.entry-title')?.textContent.replace('Nonton Anime ', '').trim() || 'N/A';
        data.link_poster = infoSection.querySelector('.thumb img')?.src || 'N/A';

        // Rating
        const rtg = infoSection.querySelector('.rtg');
        if (rtg) {
            const ratingValue = rtg.querySelector('span[itemprop="ratingValue"]')?.textContent.trim();
            const ratingCount = infoSection.querySelector('i[itemprop="ratingCount"]')?.getAttribute('content');
            data.rating = `${ratingValue} / 10 (${ratingCount} Votes)` || 'N/A';
        }

        // Deskripsi & Genre
        const descElements = infoSection.querySelectorAll('.entry-content-single[itemprop="description"] p');
        data.deskripsi = Array.from(descElements).map(p => p.textContent.trim()).join('\n\n');
        ...
        document.querySelectorAll(DETAIL_AREA_SELECTOR).forEach(detail => {
            ...
            if (strongElement) {
                ...
                value = detail.textContent.replace(strongElement.textContent, '').trim();
